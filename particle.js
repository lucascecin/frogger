class Particle {
    constructor(x, y){
        this.x = x + 25;
        this.y = y + 25;
        this.radius = Math.random() * 20 + 1;   // aleatório entre 1 e 20
        this.opacity = 1;
        this.directionX = Math.random() * 1 - 0.5; // aleatório entre -0.5 e 0.3999
        this.directionY = Math.random() * 1 - 0.5;
    }
    draw(){
        ctx3.fillStyle = 'rgba(150,150,150,' + this.opacity + ')'; //string concatenation 
        ctx3.beginPath();
    ...
    drawRipple(){
        ctx1.strokeStyle = 'rgba(255,255,255,' + this.opacity + ')'; //string concatenation 
        ctx1.beginPath();
...
function handleParticles(){
    ...
    if (particlesArray.length > maxParticles){
        for (let i = 0; i < 30; i++){ // se tem mais 300 particles, remove 30 
            particlesArray.pop();   // pop remove o último item do array 
        }
    }
...
function handleRipples(){
    ...
    if (ripplesArray.length > 20){
        for (let i = 0; i < 5; i++){ // se tem mais 300 particles, remove 30 
            ripplesArray.pop();   
        }
    }
